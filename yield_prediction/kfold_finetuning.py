...
def train_loop(train_ds, valid_ds=None):
    kfold = KFold(n_splits=5, shuffle=True, random_state=CFG.seed)
    
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_ds)):
        LOGGER.info(f"Training Fold {fold+1}/{CFG.epochs}")
        
        train_data = train_ds.iloc[train_idx]
        val_data = train_ds.iloc[val_idx]
        
        train_dataset = TrainDataset(CFG, train_data)
        valid_dataset = TrainDataset(CFG, val_data)
        ...
        for epoch in range(CFG.epochs):
            ...
            LOGGER.info(f'Fold {fold+1} Epoch {epoch+1} - avg_train_loss: {avg_loss:.4f}  val_rmse_loss: {val_loss:.4f}  val_r2_score: {val_r2_score:.4f}  time: {elapsed:.0f}s')
        
            if val_loss < best_loss:
                es_count = 0
                best_loss = val_loss
                LOGGER.info(f'Fold {fold+1} Epoch {epoch+1} - Save Lowest Loss: {best_loss:.4f} Model')
                torch.save(model.state_dict(), OUTPUT_DIR + f"finetuned_model_fold{fold+1}.pth")
                
            else:
                es_count += 1
                if es_count >= CFG.patience:
                    LOGGER.info(f'Fold {fold+1} Epoch {epoch+1} early_stopping')
                    break
